...
----
include::{go-code}/sort/bubblesort/main.go[]
----
...
此处介绍的折半插入,其实就是通过不断地折半来快速确定第i个元素的插入位置,这实际上是一种查找算法: 折半查找.Java 的 `Arrays` 类里的 `binarySearch()` 方法,就是折半查找的实现,用于从指定数组中查找指定元素,前提是该数组已经处于有序状态.
